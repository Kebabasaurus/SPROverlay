...
def draw_fuel_display(surface, center, radius, fuel_level, fuel_per_lap):
    ...
    # Draw the fuel per lap below the fuel level
    fuel_per_lap_text = font.render(f"F/L: {fuel_per_lap:.3f}%", True, (255, 230, 255))
    text_rect_per_lap = fuel_per_lap_text.get_rect(center=(center[0], center[1] + radius + 20))
    surface.blit(fuel_per_lap_text, text_rect_per_lap)
...
# Variables to store previous lap data
previous_lap = 0
...
while running:
    ...
    if ir['LapNumber'] != previous_lap:
        fuel_per_lap = previous_fuel_level - fuel_level
        previous_fuel_level = fuel_level
        previous_lap = ir['LapNumber']
    else:
        previous_fuel_level = fuel_level
    ...
    # Draw the fuel display with fuel per lap
    draw_fuel_display(screen, fuel_center, fuel_radius, fuel_level, fuel_per_lap)
