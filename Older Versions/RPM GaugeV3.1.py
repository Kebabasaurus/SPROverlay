...
# Define throttle bar parameters
throttle_bar_width = 60
throttle_bar_height = 400
throttle_bar_x = 20
...
def draw_throttle_bar(surface, x, y, width, height, throttle_percentage):
    # Draw the background of the throttle bar
    pygame.draw.rect(surface, (50, 50, 50), (x, y, width, height))
    pygame.draw.rect(surface, (255, 255, 255), (x, y, width, height), 2)  # Border
    
    # Draw the throttle fill based on percentage
    fill_height = (throttle_percentage / 100) * height
    pygame.draw.rect(surface, (0, 255, 0), (x, y + height - fill_height, width, fill_height))

    # Render the throttle percentage text
    font = pygame.font.Font(None, 24)
    throttle_text = font.render(f"{throttle_percentage:.0f}%", True, (255, 255, 255))
    text_rect = throttle_text.get_rect(center=(x + width / 2, y + height / 2))
    surface.blit(throttle_text, text_rect)
...
while running:
    ...
    draw_throttle_bar(screen, throttle_bar_x, throttle_bar_y, throttle_bar_width, throttle_bar_height, throttle_percentage)
