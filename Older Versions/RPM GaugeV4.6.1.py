...
def set_window_mode(borderless):
    ...
    if borderless:
        ...
        ex_style = win32gui.GetWindowLong(hwnd, win32con.GWL_EXSTYLE)
        win32gui.SetWindowLong(hwnd, win32con.GWL_EXSTYLE, ex_style & ~win32con.WS_EX_LAYERED & ~win32con.WS_EX_TRANSPARENT)
        # Reset the window to its normal appearance
        win32gui.SetLayeredWindowAttributes(hwnd, 0, 0, win32con.LWA_COLORKEY | win32con.LWA_ALPHA)
...
def handle_keydown(event):
    ...
    global borderless
    if event.key == pygame.K_F6 and pygame.key.get_mods() & pygame.KMOD_CTRL:
...
while running:
    ...
    # Create invisible rectangles for UI items
